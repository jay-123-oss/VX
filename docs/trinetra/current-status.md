...
- Thermal policy now exposes a Hindi search cooldown; the UI shows bounded countdown/status text while preserving Reflex Shield priority. The user-triggered ONNX Search Engine now selects HIGH/BALANCED/LOW profiles from thermal state/headroom, enforces a minimum inference interval, and downsamples YUV during search JPEG encoding so no full-resolution search buffer is allocated under hot profiles. Reflex Shield depth input is unchanged.
